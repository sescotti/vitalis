...
public class AlertController  extends AbstractApiController {
    ...
    @RequestMapping(method = POST, value = "/{alertId}", consumes = "application/json", produces = "application/json")
    public ResponseEntity<?> addModule(@ModelAttribute("user") User user,
                                       @PathVariable("alertId") Long alertId,
                                       @RequestBody AlertRequest request){
        try {
            Alert alert = alertService.modifyAlert(alertId, request.getMonitoringId(), request.getMeasurementType(), request.getFrom(), request.getTo(), user);
            return new ResponseEntity<>(alert, OK);
        } catch (IllegalArgumentException e) {
            return new ResponseEntity<>("{\"error\": \"" + e.getMessage() + "\"}", BAD_REQUEST);
        } catch (Exception e) {
    ...
    @RequestMapping(method = POST, value = "/{alertId}/delete", produces = "application/json")
    public ResponseEntity<?> addModule(@ModelAttribute("user") User user,
                                       @PathVariable("alertId") Long alertId){
        try {
